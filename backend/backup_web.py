...
def batch_upsert_tracks(rows):
    """Upsert a batch of track records into the Supabase 'tracks' table."""
    if not rows:
        return
    try:
        _supabase.table("tracks").upsert(rows, on_conflict="spotify_uri").execute()
        logging.info(f"Upserted {len(rows)} tracks to Supabase")
    except Exception as e:
        logging.error(f"Failed to upsert tracks: {e}")
        raise

def build_track_data_from_csv(row_dict: dict) -> dict:
    """
    Convert a single CSV row (dict with CSV column names) into a dict
    ready for upsert into the 'tracks' table.
    Returns None if spotify_uri is missing.
    """
    spotify_uri = str(row_dict.get("Track URI", "")).strip()
