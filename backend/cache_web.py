...
def get_cached_link(spotify_uri):
    """
    Retrieve cached YouTube information for a Spotify URI.
    
    Returns:
        tuple: (youtube_link, duration_seconds, title) or (None, None, None) if not found.
    """
    try:
        res = _supabase.table("links").select("youtube_link, duration_seconds, title")\
...
def batch_insert_links(rows):
    """rows: list of dicts with song,artist,spotify_uri,youtube_link,username,duration_seconds,fetched_at,Album Name,title"""
    if not rows:
        return
    _supabase.table("links").upsert(rows, on_conflict="spotify_uri").execute()

def prepare_link_row(song, artist, spotify_uri, link, username, duration_seconds, album=None, title=None):
    """
