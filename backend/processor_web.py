...
def add_log(task_id, msg):
    ...
    if task:
        logs = task.get('logs', [])
        logs.append(msg)
        if len(logs) > 1000:          # keep only last 1000
            logs = logs[-1000:]
...
def process_csv(input_path, output_path, settings, username, task_id):
    ...
    if MATCH_PASS_COL not in df.columns:          # NEW: add match pass column
        df[MATCH_PASS_COL] = 0
    ...
    for idx, row in df.iterrows():
        ...
        # --- Check local cache first (now returns 4 values: link, duration, title, match_pass) ---
        link, cached_dur, cached_title, cached_pass = get_cached_link(spotify_uri)
        if link:
            ...
            df.at[idx, MATCH_PASS_COL] = cached_pass if cached_pass is not None else 0
            new_links += 1
            add_log(task_id, f"Inserted (cache): {song}")
        else:
            try:
                # search_youtube now returns 4 values
                link, api_dur, api_title, pass_num = search_youtube(
            ...
            if link:
                ...
                df.at[idx, MATCH_PASS_COL] = pass_num if pass_num is not None else 0
                new_links += 1
                add_log(task_id, f"Inserted: {song} (pass {pass_num})")

                # Build full row dict for the links table (include match_pass)
                link_row = {
                    "song": song,
                    "artist": artist,
                    ...
                    "title": api_title,
                    "match_pass": pass_num   # NEW
                }
                add_pending_link(spotify_uri, link, api_dur, api_title, link_row, pass_num)
