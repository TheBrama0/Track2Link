...
def search_youtube(song, artist, album, api_key, target_duration_sec=None, max_results=50, retries=5):
    """
    Search YouTube for a track. Retries on HTTP 429 (rate limit) with exponential backoff + jitter.
    ...
    # Build query: optionally include album if it helps (but keep it clean)
    clean_artist_name = clean_artist(artist)
    query = f"{clean_artist_name} {song}".strip()
    if album and album.strip() and album.lower() != 'nan':
        # Add album only if it's short enough (prevents overly long queries)
        album_clean = album.strip()[:60]
        query += f" {album_clean}"
