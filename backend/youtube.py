...
def search_youtube(song, artist, album, api_key, target_duration_sec=None, max_results=50, retries=5):
    ...
    for attempt in range(retries):
        try:
            ...
            # ----- Very loose scoring (substring matching, no penalty) -----
            def score_video_loose(title, channel_title):
                title_lower = title.lower()
                channel_lower = channel_title.lower()
                matched = sum(1 for w in song_words if w in title_lower)
                score = min(200, matched * 50)
                if re.search(rf'{re.escape(clean_artist_name.lower())}', title_lower):
                    score += 30
                if re.search(rf'{re.escape(clean_artist_name.lower())}', channel_lower):
                    score += 10
                if "official" in title_lower or "music video" in title_lower:
                    score += 20
                if title_lower == f"{clean_artist_name.lower()} - {song.lower()}":
            ...
            if candidates1:
                ...
                return f"https://www.youtube.com/watch?v={best[0]}", best[2], best[1], 1
            ...
            if candidates2:
                ...
                return f"https://www.youtube.com/watch?v={best[0]}", best[2], best[1], 2

            # ----- PASS 3 (Very loose) -----
            logging.info(f"No mid match. Trying very loose pass (artist optional, substring matching).")
            candidates3 = []
            for video_id, title, channel_title in video_metadata:
                duration = durations.get(video_id)
                if duration is None:
                ...
                if is_blacklisted_version(title, song):
                    continue
                title_lower = title.lower()
                # Must contain at least one song word (substring)
                if not any(w in title_lower for w in song_words):
                    continue
                if target_duration_sec is not None and abs(duration - target_duration_sec) > 30:
                    continue
                score, matched = score_video_loose(title, channel_title)
                candidates3.append((video_id, title, duration, score, matched, channel_title))

            if candidates3:
                candidates3.sort(key=lambda x: (-x[3], abs(x[2] - target_duration_sec) if target_duration_sec else 0))
                best = candidates3[0]
                logging.info(f"Very loose match for '{song}': '{best[1]}' (score {best[3]}, matched {best[4]})")
                return f"https://www.youtube.com/watch?v={best[0]}", best[2], best[1], 3
