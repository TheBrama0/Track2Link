...
def search_youtube(song, artist, album, api_key, target_duration_sec=None, max_results=50, retries=3):
    """
    Search YouTube for a track. Automatically retries on HTTP 429 (per-minute rate limit).
    ...
    for attempt in range(retries):
        try:
            ...
            logging.info(f"Searching YouTube with query: '{query}' (attempt {attempt+1}/{retries})")
            ...
            if e.resp.status == 429:
                wait = (2 ** attempt) * 0.5  # 0.5, 1.0, 2.0 seconds
                logging.warning(f"Rate limited (429). Retrying in {wait}s... (attempt {attempt+1}/{retries})")
                time.sleep(wait)
                if attempt == retries - 1:
                    raise Exception("YouTube API per-minute quota exceeded after retries.")
                continue  # retry the same search
